...
\begin{proof}
    ...
    $K_m$ coloured in $K_m$ coloured with the $i^{\text{th}}$ colour. Hence, \cite{Bollobas1998}
...
\begin{proof}
    Notice that for $n=1$, we already know that $r(K_3)=R(3,3)=6$. We first show 
    that $r(nK_3)\geq5n$. Consider the 2-colouring of $K_{5n-1}$ in which we 
    form red triangles from $3n-1$ vertices, blue triangles from the remaining $2n$ vertices, 
    and colour every other edge blue. The only monochromatic triangles here are 
    non-disjoint, so this colouring contains no monochromatic $nK_3$, and 
    so $r(nK_3)>5n-1$.\\

    Now we show, by induction on $n$, that $r(nK_3)\leq5n$ for $n\geq2$. The base 
    case $n=2$ requires detailed and unenlightening analysis, which we omit. Now fix 
    an arbitrary 2-colouring of $K_{5n}$, for $n\geq3$.
    We may find $\frac{5n-5}{3}$ disjoint monochromatic triangles by 
    simply selecting triangles until fewer than six vertices remain. As 
    $\frac{5n-5}{3}\geq n$, we may assume that at least one of these triangles 
    is in each colour, say $\{1,2,3\}$ is coloured red and $\{4,5,6\}$ is coloured 
    blue.\\

    By `bow-tie' we mean a pair of triangles of different colours that share a 
    single common vertex. Assume, by symmetry, that at least five of the edges 
    ...
    $\{1,2,3,4,5\}$ forms a bow-tie. When these five points are removed, there exists, 
    by induction, a monochromatic $(n-1)K_3$ on the remaining $5n-5$ points. This, 
    together with the appropriate $K_3$ in the bow-tie, gives a monochromatic $nK_3$.
    \cite{Graham1990}
\end{proof}

\textbf{Remark}: This proof can be generalised to find the generalised Ramsey 
numbers $r(sG,tG)$, for an arbitrary graph $G$ and $s,t\in\Z^+$. For example, if $s\geq t\geq1$, then,
$$r(sK_2,tK_2)=2s+t-1,$$
