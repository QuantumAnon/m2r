...
\begin{proof}
    ...
    replace the first two colours by a new colour. So long as $n$ is large enough,
    ...
    we have either a monochromatic $K_{s_1}$ or a monochromatic $K_{s_2}$ as a clique of $K_m$ by \autoref{prop:ram}, 
...
\begin{proof}
    ...
    $$S_1=\{x\in\Gamma\setminus\{a_1\}:\chi(\{a_1,x\})=c_1\}$$
...
\begin{theorem}
    ...
    $$r(nK_3)=5n\qquad \text{for  } n\geq1.$$
\end{theorem}

\begin{proof}
    Notice that for $n=1$, $r(K_3)=R(3,3)=6>5$. Now we first show 
