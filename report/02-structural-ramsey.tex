...
\begin{definition}[Homomorphisms and Substructures\cite{Hubicka2015,Hubicka2016}]
...
\begin{prop}\label{prop:EPIso}
  ...
  \begin{proof}
   ...
   We can now begin the construction of the full isomorphism by a method known
   as \textit{back and forth}.
...
structure that satisfies the \hyperlink{rem:extprop}{EP} and that it is unique
up to isomorphism.
...
$\str{A}\in\gra$ is a finite structure over the language $(\{1\},(2))$
...
\subsection{Ramsey Classes}\label{sec:4.4}
...
\begin{definition}[Ramsey Class\cite{Nesetril1995}]
  ...
  $\str{B}\in\class{K}$ which $\str{A}$ will embed into and for every
  $t\in\Z^+$, there exists $\str{C}\in\class{K}$ such that 
...
In fact, a direct corollary of the finite Ramsey theorem from the original
paper~\cite[267]{Ramsey1930} is that the class of finite complete graphs is a
Ramsey class.

One might hope that this property could be extended to $\gra$, but this is not
the case as given by the result below
\begin{theorem}[Ramsey property in $\gra$\cite{Nesetril1995}\label{thr:ramsPropGra}]
  $\gra$ has the $\str{A}$-Ramsey property $\iff$ $\str{A}$ is either a complete
  graph or the complement of a complete graph (empty graph).
\end{theorem}
This is a non-trivial result whose proof we omit, though we refer the reader
to~\cite[1375-1377]{Nesetril1995} for a rigorous argument.
...
\begin{remark}
  ...
  Then $\str{C'}\radoArrow{B'}{A}{t} \implies \exists \str{C}\in\ogra :
  \str{C}\radoArrow{B}{A}{t}$ by replacing  every $B'\in\sstr{C'}{B'}$ with a
  full copy of $B$.
\end{remark}
...
\begin{theorem}
  ...
  \begin{proof}[proof\ sketch.]
    ...
    $\str{C}\radoArrow{B}{A}t$.
    We state that $C\ni x_i \in B^N$ where $N$ is some, currently undetermined,
    positive integer.
    ...
    $\str{A}_i$ all of which are substructures of $\str{B}$ which completes the proof.
  \end{proof}
\end{theorem}
...
\begin{definition}[$G$-flows and Extreme Amenability\cite{Kechris2005}]
  ...
  If the universal minimal flow is a single point, we say $G$ is \textit{extremely amenable}.
\end{definition}
...
\begin{theorem}[\citeauthor{Kechris2005}~\cite{Kechris2005}]
  Let $\class{K}$ be a Fraïssé class and $\str{K}$ its limit.
