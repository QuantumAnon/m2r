...
\begin{theorem}
  ...
  \begin{proof}[Sketch of proof]
    ...
    $\str{A}_i$ all of which are substructures of $\str{B}$ which completes the
    proof.
  \end{proof}
\end{theorem}
...
\begin{definition}[$G$-flows and Extreme Amenability\cite{Kechris2005}]
  ...
  If the universal minimal flow is a single point, we say $G$ is
...
With this we can start to link automorphisms of $\ogra$ to flows on
topological spaces.
It is shown in~\cite[130]{Kechris2005} that these automorphisms are subgroups of
the Polish group on $\Z^+$
