...
% Order the theorem names by date
%% I don't think this makes sense since this chapter derives gallai-witt and van der waerden from hales-jewett despite the latter being proven after the former
...
one given by \textcite{Shelah1988}, which gives a primitive-recursive upper bound.
...
The theorem of \textcite{Waerden1971} was actually stated and proved 
some time before Hales and Jewett's paper, but can be considered 
as a direct result of the more general Hales-Jewett theorem \cite{Jukna2013}.
It states the following:

\begin{theorem}[van der Waerden]\label{thm:vanw}
For any $r, n \in \Z^+$, there exists an 
$N \in \Z^+$ such that for any r-colouring of the set 
...
\begin{theorem}[Gallai-Witt \cite{Schacht2011}]
...
van der Waerden's, and is thus omitted.


\subsection{Szemerédi's theorem and the Density Hales-Jewett theorem}
...
\emph{density} in a set.
For a set $B$, the density of a subset $A\subset B$ is
$\frac{|A|}{|B|}$.
...
theorem, respectively, and just as with the original theorems, Szemerédi's
theorem can be easily deduced from the density Hales-Jewett theorem.
...
If $A^N$ is $r$-coloured, then at least one of these subsets 
...
\textcite{Furstenberg1991} using ergodic theory, the theory of
measure-invariant dynamical systems.
It used techniques developed in Furstenberg's proof of Szemerédi's 
theorem, which considered the Bernoulli system, a dynamical system 
on the set of subsets of the integers under the operation of \textquotedblleft shifting" 
subsets by 1.
A more recent and elementary proof was produced by the Polymath 
project \cite{Polymath2009}, which proved the theorem by contradiction using the strategy
...
$[k]^N$, or $\mathbb{Z}$ for the proof of Szemerédi's), and does not 
...
a substructure of the same kind where $A$ has density $\delta + c + \gamma$, where $c$ and $\gamma$ depend only on $\delta$.
From a large enough structure, this iteration can be continued 
until $A$ has a density greater than $1$ in some substructure, 
a contradiction.

In fact, it is often difficult to find such substructures $S'$, so instead one 
finds a \textquotedblleft simple" (in some well defined sense) subset $T$ in which $A$ has density
$\delta + c$, and then proves that such simple sets may be partitioned into substructures 
of the desired type such that each member of $T$ is contained in approximately the same 
number of substructures, and finally uses an averaging argument to show that $A$ has 
density $\delta + c$ in at least one of these substructures.

Part of the motivation behind finding a new proof of the DHJ theorem is the fact that 
Furstenberg's proof used non-finitary ergodic methods reliant on the axiom of choice, 
which meant that it could not be used to construct an upper bound for a DHJ 
number\cite{Furstenberg1991}. The Polymath project's proof, on the other hand,
was finitary, meaning it could be formulated as a finite set of propositions
reliant on a finite set of axioms, and thus gave a method for finding upper bounds 
for the number, albeit bounds of massive order.

\paragraph*{}
Of course, none of these proofs are constructive of their respective numbers,and thus 
our knowledge of the distribution of Hales-Jewett numbers, van der Waerden numbers and
Gallai-Witt numbers continues to expand as computational power and theoretical methods
improve. As they do, our understanding of the fields connected to these theorems expands 
as well. The existence of numerous proofs of the Density Hales-Jewett theorem and related 
results highlights a theme common across Ramsey theory; that of connecting seemingly 
unrelated areas of mathematics using the universal language of combinatorics. 

In fact, Terrence Tao once called the various proofs of Szemeredi's theorem a \textquotedblleft Rosetta stone"
for connecting disparate areas of mathematics\cite{math/0512114}. We can also see this on a 
simple scale in the relationships between the Hales-Jewett theorem and those of Van der 
Waerden and Gallai, how a fact about columns in a cube or words in an alphabet can imply 
results about the natural numbers or vectors. We will see in the next section how the principles
developed in the prior chapters can be connected with the study of Euclidean space, highlighting
once more how Ramsey theory can relate properties of structures to those of their underlying 
sets, and thus create relationships between properties of the structures themselves.
