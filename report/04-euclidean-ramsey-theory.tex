...
$k\in\mathbb{Z}^+$. Then an element $x = (x_{1},\cdots,x_{n})\in X^n$ is 
...
\in \spa\bigg\{g_{j} \ \bigg| \ j\in\bigcup\limits_{k \in [r]: k < i }\mathcal{C}_{k}\bigg\}$
\end{enumerate}
...
the first of which results almost immediately from application of Gram-Schmidt orthogonalization:

\begin{lemma}\label{lem:vec}
Let $\{v_{1},\cdots,v_{n},v\}\in \mathbb{Z}^m$. If $v \not\in \spa\{v_{1},\cdots,v_{n}\}$, 
then:
\begin{center}
    $\exists u\in\mathbb{Z}^m$ such that: $u\cdot v\not = 0$ and $\forall i\in [n], u\cdot v_{i}=0$.
\end{center}
...
\end{center} 
and for a given $y\in \mathbb{N}^{m+1}$ an \emph{(m,p,c)-set} is defined as:
$$S_{m,p,c}(y):=\bigg\{y\cdot n \ | n\in N_{m,p,c}\bigg\}$$
...
As there exist infinitely many prime integers $p$ in $\mathbb{N}$, 
the set $\{\Pi^*_{p}\ : \ p$ is prime in $\mathbb{N}\}$
...
with an algebraic constraint on Ramsey sets in $\mathbb{N}$.
...
of Euclidean space into a geometrical property. In particular we are now interested
in affine linear equations on $\mathbb{R}$ that do not admit monochromatic solutions 
(Lemma \ref{lem:radreal}).
These allow us to formulate a necessary geometrical condition for a finite 
...
A natural question to consider is whether there exists a class of 
...
the same configuration is spherical. In fact isometries preserve spheres. 
...
exist scalars $\{\mu_{1},\cdots, \mu_{k},b\}\subset\mathbb{R}$ not all zero 
such that:
...
the equations (i) and (ii) are invariant under isometry in $\mathbb{R}^n$.
